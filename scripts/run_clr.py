...
def mobile_get_all():
    ...
    if cid is None:
        ...
        try:
            ...
            cur = db.cursor()
            query = (
            " SELECT t2.ProdType,t3.ProdImg from [CIDB].[DataMart].[OFFER] as t1 inner join [CIDB].[DataMart].[PRODUCT] as t2 on t1.ProdID = t2.ProdID "
            " inner join [CIDB].[DataMart].[ProdImg]  as t3 on t2.ProdID = t3.ProdID")
            cur.execute(query)
            ...
            for row in result_mssql_offers:
                ...
                offer["name"] = row[0]
                offer["duration"] = 12
                offer["type"] = prodtype[i][0]#'financial'#row[9]
...
def freshmeatprods():
    ...
    if is_newcust == []:
        try:
            ...
            select_offer_temp = ("Select * from [DataMart].[OFFER_TEMPLATE]")
...
'TransInfo':"atmerror",'TransParam1':'','TransParam2':'','TransParam3':'','TransParam4':''}
        ...
        payload = {"name":"","surname":"","middlename":"","dob":"","id":int(CID),"status":"","reason":"Withdrawal","location":"ATM","area":""}
        try:
