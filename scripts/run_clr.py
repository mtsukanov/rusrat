...
@crossdomain(origin='*', content = 'application/json')
def offer_accept():
    try: 
...
"param6" : None,
"param7" : None}
            return make_response(jsonify({'Ratatoskr':inputs}),418)
        except Exception:
            return make_response(jsonify({'Ratatoskr':'error processing mobile'}),418)       
    else:
        try:
            cid = request.json['cid']
            ...
            return make_response(jsonify({'Ratatoskr':'error in inputs'}),418) 
        try:
            inputs = {"cid":cid,
"channel": channel,
