...
def post(maxevent):
    ...
    while i==1:
        ...
        for row in cur.fetchall():
            if row[2] > 85.00:
                payload = {"name":row[3],"surname":row[4],"middlename":"Vasya","dob":"1990-11-10","id":row[5],"status":"ok","reason":"mortgage","location":"camera","area":"retail"}
                r = requests.post("http://172.28.104.171:5000/active_queue",json = payload)
