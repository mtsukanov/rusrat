...
def mysql_add():
    db_='thebankfront'
    db = MySQLdb.connect(host=mysqlpath, port = 3306, user="rusrat",passwd="Orion123", db=db_)
    cur = db.cursor()
...
def call_service(service,inputs):
    service_adr = "http://10.20.1.21:5000/"+service
    payload = inputs
...
def post(maxevent):
    ...
    while i==1:
        ...
        try:
            ...
            return e
        cur = db.cursor()
        query = "SELECT event_id,event_time,similarity,first_name,last_name,middle_name,event_photo,birth_date FROM event WHERE search_result ='TRUE' event_id >"+str(maxid)
        cur.execute(query)
        ...
        for row in cur.fetchall():
            ...
            r1 = requests.put("http://10.20.1.21:5000/active_queue?option=terminal",json = payload1)
            ...
            r2 = requests.post("http://10.20.1.21:5000/active_queue",json = payload2)
